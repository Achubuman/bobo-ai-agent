...
👉 **[開啟一條龍整合 Demo](index.html)**（部署後，GitHub Pages 首頁即此 demo）— 模擬「在 LINE 裡」的完整流程，一支手機走到底：

群組 @波波 揪團 → AI 推薦店家 → 團主在私訊編輯菜單（菜單 OCR）→ 一鍵開團 → 成員點餐 → 缺貨自動通知改單 → 截單 → 收款對帳。

> 用瀏覽器開啟，拉成手機寬度即為 LINE 內樣貌。聊天為 LINE 原生模擬，菜單 OCR／開團面板／點餐頁皆為內嵌網頁（對應實際的 LINE LIFF）。**全程免金鑰、可離線。**
